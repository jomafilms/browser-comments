...
- **Single-ticket endpoint:** `GET /api/comments/[id]` now returns the **full record** (accepts serial id, uuid, or ref like `LWF-12`, scope-checked). `?includeImage=true` adds `image_data`; `?imageOnly=true` returns the legacy `{image_data}` shape.
